            ...
            <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
                        ...
                        <div class="flex items-center gap-5">
                            <div class="w-16 h-16 bg-gray-50 rounded-2xl flex-shrink-0 flex items-center justify-center border border-gray-100 font-bold text-gray-400">
                                @if($item->barang && $item->barang->foto_produk)
                                    <img src="{{ asset($item->barang->foto_produk) }}" class="w-full h-full object-cover rounded-2xl">
                                @else
                ...
                <div class="flex gap-3 w-full md:w-auto">
                    @if(strtolower($order->status ?? '') === 'belum_bayar')
                        <button class="flex-1 md:flex-none bg-red-600 hover:bg-red-700 text-white font-bold px-8 py-4 rounded-2xl shadow-xl shadow-red-100 transition-all transform hover:-translate-y-1">
                            Lakukan Pembayaran
                        </button>
                    @elseif(strtolower($order->status ?? '') === 'dikirim')
                        <button class="flex-1 md:flex-none bg-gray-900 hover:bg-black text-white font-bold px-8 py-4 rounded-2xl shadow-xl shadow-gray-200 transition-all transform hover:-translate-y-1">
                            Lacak Pengiriman
                        </button>
                    @endif
                </div>
